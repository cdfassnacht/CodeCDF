...
class ccdSet(object):
    ...
    Code that can be used process a set of one type of imaging files, such as
    bias, flat, or science frames.

    """

    # -----------------------------------------------------------------------

    def __init__(self, infiles, bpmfiles=None, maskfiles=None, varfiles=None,
                 gain='gain', texp='exptime', coadd=None, x1=0, x2=0,
                 y1=0, y2=0):
        """

        Load the input data files and associated calibration files

        """

        self.nfiles = len(infiles)
        self.infiles = infiles
        self.bpmfiles = bpmfiles
    ...
    def load_data(self):
        ...
        Reads in files containing the imaging data (science frames, bias
        frames, flat-fields, etc.) and, optionally, the associated
        variance and/or bad-pixel and/or image mask files
        ...
        match_error = False
        auxlist = [self.maskfiles, self.bpmfiles, self.varfiles]
        auxnames = ['Mask', 'BPM', 'Variance']
        naux = np.ones(len(auxlist)) * -1.

        nfiles = len(infiles)
        for i in range(len(auxlist)):
            if auxlist[i] is not None:
                naux[i] = len(auxlist[i])
                if naux[i] != nfiles:
                    match_error = True
        ...
        if match_error:
            ...
            print('  Image files: %d' % nfiles)
            for i in range(len(auxlist)):
                if auxlist[i] is not None:
                    print('  %s files: %d' % (auxnames[i], naux[i]))
            print('')
   ...
   # -----------------------------------------------------------------------
    ...
    def comb_whtave(self, infiles, weights, outfile=None):
       """
       Does a weighted average of an image stack.  The final image will
       be given by:
       ...
       Note that the weights can either be scalars or 2-d images that match
