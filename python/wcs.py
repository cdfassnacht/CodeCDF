...
def make_header(ra, dec, xsize, ysize, xscale, yscale=-1, docdmatx=True):
   ...
   hdr["CRPIX2"] = ysize/2.
   if docdmatx:
      hdr['cd1_1'] = -xscale/3600.
      hdr['cd1_2'] = 0.
      hdr['cd2_1'] = 0.
      hdr['cd2_2'] = yscale/3600.
   else:
      hdr["CDELT1"] = xscale/3600.
      hdr["CDELT2"] = yscale/3600.
      hdr["PC1_1"] = -1.
      hdr["PC1_2"] = 0.
      hdr["PC2_1"] = 0.
      hdr["PC2_2"] = 1.
...
def make_hdr_astropy(ra, dec, xsize, ysize, xscale, yscale=-1):
   ...
   hdr = w.to_header()
   """
   The to_header method does not write the PC matrix (PC1_1, PC1_2, etc.)
   to the header if it is the unit matrix, which it will be in this situation.
   Since other functions expect a PC matrix, add it in.
   """
   hdr["PC1_1"] = -1.
   hdr["PC1_2"] = 0.
   hdr["PC2_1"] = 0.
   hdr["PC2_2"] = 1.
   """ 
   Also add an RA and Dec parameters, as if this came from a telescope
   pointing.
   """
   hdr['ra'] = ra
   hdr['dec'] = dec
...
# Apply a rotation to a wcs
def rotate_header(oheader,angle):
   DEGRAD = pi/180.
   header = oheader.copy()
   header['PC1_1'] = cos(DEGRAD*angle) * oheader['PC1_1'] + \
       sin(DEGRAD*angle) * oheader['PC2_1']
   header['PC1_2'] = cos(DEGRAD*angle) * oheader['PC1_2'] + \
       sin(DEGRAD*angle) * oheader['PC2_2']
   header['PC2_1'] = -1. * sin(DEGRAD*angle) * oheader['PC1_1'] + \
       cos(DEGRAD*angle)*oheader['PC2_1']
   header['PC2_2'] = -1. * sin(DEGRAD*angle) * oheader['PC1_2'] + \
       cos(DEGRAD*angle)*oheader['PC2_2']
   return header
