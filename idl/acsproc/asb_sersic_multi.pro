;+
;
; NAME: asb_sersic_multi
;
; PURPOSE: Generate a 2d Sersic with specified parameters.
;
; USAGE:
;   image = asb_sersic_multi(x, y, p [, basis=basis, rmin=rmin, $
...
; ***WARNING: uses deVaucouleurs constant for all values of n!!!***
;     p[2] = x-center
;     p[3] = y-center
;     p[4] = axis ratio
...
; back to Sersic form 2007jun abolton@cfa
;
;-

function asb_sersic_multi, x, y, p, basis=basis, rmin=rmin, psf=psf, $
...
;  if (min(r_ell) le 0.) then stop
  z_this = a * exp(-k_dev*r_ell^(1./nser))
