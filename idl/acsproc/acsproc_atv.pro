...
;
; CALLING SEQUENCE:
;       acsproc_atv [,array_name OR fits_file] [,min = min_value] [,max=max_value] 
;           [,/linear] [,/log] [,/histeq] [,/block]
;           [,/align] [,/stretch] [,header = header]
;
; REQUIRED INPUTS:
;       None.  If acsproc_atv is run with no inputs, the window widgets
;       are realized and images can subsequently be passed to acsproc_atv
...
; 
; COMMON BLOCKS:
;       acsproc_atv_state:  contains variables describing the display state
;       acsproc_atv_images: contains the internal copies of the display image
;       acsproc_atv_color:  contains colormap vectors
;       acsproc_atv_pdata:  contains plot and text annotation information
;
; RESTRICTIONS:
...
;
; EXAMPLE:
;       To start acsproc_atv running, just enter the command 'acsproc_atv' at the idl
;       prompt, either with or without an array name or fits file name 
;       as an input.  Only one acsproc_atv window will be created at a time,
;       so if one already exists and another image is passed to acsproc_atv
;       from the idl command line, the new image will be displayed in 
;       the pre-existing acsproc_atv window.
;
...
;
;       Refactored into a subprogram of hstproc for GUI management
...
;              http://www.astro.caltech.edu/~barth/acsproc_atv
;
;       Hacked up by Finkbeiner 5 December 2003 to support healpix.
;;-
;----------------------------------------------------------------------
;        acsproc_atv startup, initialization, and shutdown routines
;----------------------------------------------------------------------

pro acsproc_atv_initcommon

; Routine to initialize the acsproc_atv common blocks.  Use common blocks so
; that other IDL programs can access the acsproc_atv internal data easily.

common acsproc_atv_state, state
common acsproc_atv_point, markcoord
common acsproc_atv_color, r_vector, g_vector, b_vector
common acsproc_atv_pdata, nplot, maxplot, plot_ptr
common acsproc_atv_images, $
  main_image, $
  display_image, $
  scaled_image, $
  blink_image1, $
  ...
  unblink_image, $  
  pan_image
common acsproc_atv_data, dataroot, cycle, program, metastruc0, metastruc, metastrucindex, pixels_per_arcsec
  ...
  state = {                   $
          ...
          active_window_id: 0L, $        ; user's active window outside acsproc_atv
          info_base_id: 0L, $            ; id of base holding info bars
          nav_list_id: 0L, $             ; widget id of droplist holding navigation list
          location_bar_id: 0L, $         ; id of (x,y,value) label
          cutout_locatebar_id: 0L, $     ; id of cutout/thumbnail label widget
          cutout: 1L, $                  ; value of cutout keyword to be passed to acsproc_datafile
          combine: 0L, $                 ; value of combine keyword to be passed to acsproc_datafile
          modim: 0L, $                   ; value of modim keyword to be passed to minus_modim
          minusmodimID: -1L, $           ; value of current modimID to determine which modim to subtract (-1 for none)
          minusmodim_fit_id: 0L, $       ; widget ID of minus_modim radio buttons for searching for best modim fit
          maskstruc_nthetaID: 0L, $      ; value of maskstruc.nthetaID set by minusmodim_fit widget
          new_minusmodim: 0L, $          ; if set,  value for maskstruc.nthetaID is taken from widget (new value), otherwise maskstruc from file
          minusmodim_id: 0L, $           ; id of minus modim widget
          photmod: 0L, $                 ; value of photmod keyword to be passed to minus_modim
          ...
          messagebar_id: 0L, $           ; widget id for messagebar widget_label
          ...
          lcoord: [800,750], $           ; location x,y coord
          nav_position_id: 0L, $         ; widget id for Nav Index 
          nav_position_of_id: 0L, $      ; widget id for Nav Count 
          mode_droplist_id: 0L, $        ; widget id for Mode droplist
          imagemode_droplist_id: 0L, $   ; widget id for Image mode droplist
          capture_set_id: 0L, $          ; widget id for Set Thumbnail Checkbutton 
          capture_set: 1L, $             ; value of Set Thumbnail Checkbutton 
          capture_droplist_id: 0L, $     ; widget id for Capture mode droplist
          capturemodelist:['None','jpg','png','tiff'], $
          capturemode: 2, $              ; default capture mode=png
          maskmode: 0, $                 ; default mask mode=None
          maskpix:0, $                   ; watch for mouse drag to mask pixel area after mouse is clicked but not after mouse is released
          maskmode_droplist_id: 0L, $        ; widget id for Mask mode droplist
          limit_data_id: 0L, $           ; widget id for limit data widget
          limit_exposures_id: 0L, $      ; widget id for exposures limit widget
          limit_data: 0L, $              ; value of limit data widget (all = 0, new = 1)
          limit_exposures: 0L, $         ; value of exposures limit widget (all = 0, primary = 1)
          lens_status_id: 0L, $         ; widget id for Confirm Lens Droplist
          brush_size_id: 0L, $          ; widget id for Brush Size Droplist
          brush_size: 0L, $             ; value of mask brush size
...
function acsproc_atv_gettitle, row=this_row

  common acsproc_atv_state
  common acsproc_atv_data
  
  if not keyword_set(metastrucindex) then metastrucindex = 0
  if keyword_set(this_row) then row = this_row else row = metastrucindex
  
  if (size(metastruc, /tname) eq 'STRUCT') and (row ge 0) then begin
    title = strtrim(metastruc[row].proposid,2) + ': '+ metastruc[row].uniqname+ ' Visit' + metastruc[row].visit+' Subexp '+strtrim(metastruc[row].subexposure,2)
  endif else if (state.imagename EQ '') then begin
      title = 'atv for hstproc'
  endif else begin
      ...
      if (slash EQ -1) then slash = strpos(state.imagename, ':', /reverse_search)
      if (slash NE -1) then title = strmid(state.imagename, slash+1)  else title = state.imagename    
  endelse
...
pro acsproc_atv_metastruc_refresh

  common acsproc_atv_state
  common acsproc_atv_data
  
  title_list = make_array(n_elements(metastruc),/string,value='hello')
  for i = 0,n_elements(metastruc)-1 do title_list[i] = acsproc_atv_gettitle(row=i)
  widget_control, state.nav_list_id, set_value=title_list
  widget_control, state.nav_position_id, set_value=metastrucindex+1
  tmp_string = ' of '+strtrim(n_elements(metastruc),2)+'     '
  widget_control, state.nav_position_of_id, set_value=strmid(tmp_string,0,10)
  image = acsproc_datafile(metastruc[metastrucindex], cutout=state.cutout, combine=state.combine)
  ;acsproc_lens_status, metastruc[metastrucindex].lens
  if file_test(image) then begin
    acsproc_atv_readfits, fitsfilename=image, newimage=newimage
    if (newimage EQ 1) then begin
      acsproc_atv_getstats
      acsproc_atv_settitle
      acsproc_atv_reset_coord
      acsproc_atv_reset_colorscale
      if state.combine then acsproc_atv_showmask
    endif 
  endif  
...
pro acsproc_atv_metastruc_limit, lcoords=lcoords

  common acsproc_atv_state
  common acsproc_atv_data
  
  if keyword_set(lcoords) then begin
    state.limit_data = 1L
    state.limit_exposures = 1L
    widget_control, state.limit_data_id, set_value=state.limit_data
    widget_control, state.limit_exposures_id, set_value=state.limit_exposures
  endif
      
  metastruc_selected = metastruc[metastrucindex]
  metastruc = metastruc0
  if (state.limit_data) then begin
    if keyword_set(lcoords) then wh=where(metastruc.statuscode eq 2,nlimit_data) else wh=where(acsproc_goodones(metastruc),nlimit_data)
    if (nlimit_data gt 0) then begin
      metastruc=metastruc[wh]
      metastrucindex0 = where(wh eq metastrucindex,c)
      if (c eq 1) then metastrucindex = metastrucindex0[0] else metastrucindex = 0
    endif else begin
      state.limit_data = 0L
      widget_control, state.limit_data_id, set_value=state.limit_data
    endelse
  endif else begin
    metastrucindex0 = where(metastruc.proposid eq metastruc_selected.proposid and metastruc.visit eq metastruc_selected.visit and metastruc.exposure eq metastruc_selected.exposure and metastruc.subexposure eq metastruc_selected.subexposure,c)
    if (c eq 1) then metastrucindex = metastrucindex0[0] else metastrucindex = 0
  endelse

  if (state.limit_exposures) then begin
    wh=where(metastruc.subexposure eq 1,nlimit_exposures)
    if (nlimit_exposures gt 0) then begin
      metastruc=metastruc[wh]
      metastrucindex0 = where(wh eq metastrucindex,c)
      if (c eq 1) then metastrucindex = metastrucindex0[0] else metastrucindex = 0
    endif else begin
      state.limit_exposures = 0L
      widget_control, state.limit_exposures_id, set_value=state.limit_exposures
    endelse
  endif else begin
    metastrucindex0 = where(metastruc.proposid eq metastruc_selected.proposid and metastruc.visit eq metastruc_selected.visit and metastruc.exposure eq metastruc_selected.exposure,c)
    if (c ge 1) then metastrucindex = metastrucindex0[0] else metastrucindex = 0
  endelse
  
  acsproc_atv_metastruc_refresh

end

;---------------------------------------------------------------------

pro acsproc_atv_startup

; This routine initializes the acsproc_atv internal variables, and creates and
; realizes the window widgets.  It is only called by the acsproc_atv main
; program level, when there is no previously existing acsproc_atv window.

common acsproc_atv_state
common acsproc_atv_color
common acsproc_atv_data
...
acsproc_atv_getwindow


; Define the widgets.  For the widgets that need to be modified later
; on, save their widget ids in state variables

base = widget_base(title = 'acsproc_atv: HSTPROC CUTOUT', $
                   xoffset=584, /column, /base_align_right, $
                   app_mbar = top_menu, $
                   uvalue = 'acsproc_atv_base', $
                   /tlb_size_events)
state.base_id = base
...
nav_base_id = widget_base(base, /row, /base_align_right)
track_base =    widget_base(base, /row)
info_base =  widget_base(track_base, /column)
min_max_base = widget_base(info_base, /row)
state.info_base_id = widget_base(info_base, /row, /base_align_right)
data_base_id = widget_base(info_base, /row, /base_align_right)
sys_base = widget_base(base, /row, /base_align_left)
buttonbar_base = widget_base(base, /column, /base_align_center)
...
state.min_text_id = cw_field(min_max_base, $
                             uvalue = 'min_text', $
                             /floating,  $
                             title = 'Min=', $
...
state.max_text_id = cw_field(min_max_base, $
                             uvalue = 'max_text', $
                             /floating,  $
                             title = 'Max=', $
...
modebase = widget_base(sys_base, /col, /base_align_left)
modebase0 = widget_base(modebase, /row, /base_align_left)
modebase1 = widget_base(modebase, /row, /base_align_left)
modebase2 = widget_base(modebase, /row, /base_align_left)
modelist = ['Color', 'Zoom','Locate', 'Mask', 'Blink', 'ImExam']
imagelist = ['FLT', 'Comb','Modim']
masklist = ['None', 'Feature', 'Junk', 'All']
modimlist = ['None', 'Moffat Fit','Bspline Cen','Bspline Fit','deVauc','Sersic']
state.mode_droplist_id = widget_droplist(modebase0, $
                                   frame = 1, $
                                   title = 'Mouse:', ysize=32, $
                                   uvalue = 'mode', $
                                   value = modelist)
state.capture_droplist_id = widget_droplist(modebase0, $
                                   frame = 1, $
                                   title = 'Capture:', ysize=32, $
                                   uvalue = 'capturemode', $
                                   value = state.capturemodelist)
state.imagemode_droplist_id = widget_droplist(modebase1, $
                                   frame = 1, $
                                   title = 'Image:', ysize=32, $
                                   uvalue = 'imagemode', $
                                   value = imagelist)
state.maskmode_droplist_id = widget_droplist(modebase1, $
                                   frame = 1, $
                                   title = 'Mask:', ysize=32, $
                                   uvalue = 'maskmode', $
                                   value = masklist)
state.minusmodim_id = widget_droplist(modebase2, $
                                   frame = 1, $
                                   title = '-:', ysize=34, $
                                   uvalue = 'minusmodim', $
                                   value = modimlist)
state.minusmodim_fit_id = cw_bgroup(modebase2, ['2','4','1'],uvalue='minusmodim_fit', /ROW,/NONEXCLUSIVE, frame = 1, SPACE=2,ysize=28)
widget_Control, state.minusmodim_fit_id, sensitive=(state.minusmodimID eq 2)
Widget_Control, state.maskmode_droplist_id, Sensitive=0
widget_control, state.capture_droplist_id, set_droplist_select=state.capturemode                                 

sys_base1 = widget_base(sys_base, /column, /base_align_left)
sys_base_row1 = widget_base(sys_base1, /row, /base_align_left)
sys_base_row2 = widget_base(sys_base1, /row, /base_align_right)
sys_base_row3 = widget_base(sys_base1, /row, /base_align_right)

tmp_string = string(1000, 1000, 1.0e-10,  format = '("(",i5,",",i5,") ",g12.5)' )
void = widget_label(sys_base_row1, value = '(X,Y) Val=',   frame = 0)
state.location_bar_id = widget_label(sys_base_row1, value = tmp_string,  uvalue = 'location_bar',  frame = 1)

resize_window_id = cw_bgroup(sys_base_row2, ['4"','5"','6"','Wide'],uvalue='resize_window', label_left='Size', /ROW, SPACE=2,ysize=28)
brush_size_base = widget_base(sys_base_row2, /row, /base_align_right, /nonexclusive)                           
state.brush_size_id = widget_droplist(sys_base_row2, $
                                   frame = 1, $
                                   title = 'Brush', ysize=34, $
                                   uvalue = 'brush_size', $
                                   value = ['1','2','3','4','5','10'])
widget_control, state.brush_size_id, set_droplist_select = 0L
widget_Control, state.brush_size_id, sensitive=0L
;cutout_locateset_button = widget_button(sys_base_row2, $
;                            value = 'Save', $
;                            uvalue = 'save')
tmp_string = 'Location'+string(10, 10, $
                    format = '("(",i5,",",i5,") ")' )
                
state.cutout_locatebar_id = widget_label(sys_base_row3, $
                                 value = tmp_string,  $
                                 uvalue = 'cutout_locatebar', frame = 1)
                            
confirm_lens_base = widget_base(sys_base_row3, /row, /base_align_right, /nonexclusive)                           
state.lens_status_id = widget_droplist(sys_base_row3, $
                                   frame = 1, $
                                   title = 'Lens', ysize=34, $
                                   uvalue = 'lens_status', $
                                   value = ['','YES','MAYBE','NO'])
widget_control, state.lens_status_id, set_droplist_select = 0L
widget_Control, state.lens_status_id, sensitive=0L


;capture_set_base = widget_base(sys_base_row3, /row, /base_align_right, /nonexclusive)    
;state.capture_set_id = widget_button(capture_set_base,value='Capture',uvalue='capture_set')
;widget_control, state.capture_set_id, set_button = 1
                                  
state.limit_data_id = cw_bgroup(data_base_id, ['All data','YES/MAYBE'],uvalue='limit_data', /COLUMN, /EXCLUSIVE,SPACE=0,ysize=57,frame=1)
state.limit_exposures_id = cw_bgroup(data_base_id, ['All exposures','primary exposures'],uvalue='limit_exposures', /COLUMN, /EXCLUSIVE,SPACE=0,ysize=57,frame=1)
widget_control, state.limit_data_id, set_value=state.limit_data
widget_control, state.limit_exposures_id, set_value=state.limit_exposures

title_list = make_array(n_elements(metastruc),/string,value='')
for i = 0,n_elements(metastruc)-1 do title_list[i] = acsproc_atv_gettitle(row=i)
state.nav_list_id = widget_droplist(nav_base_id,title='', uvalue='nav_list',value = title_list)   
state.nav_position_id = cw_field(nav_base_id,/row,title = '',uvalue='nav_position',value = '0', /string,/return_events,xsize=2)
state.nav_position_of_id = widget_label(nav_base_id,value = ' of 0     ',ysize=25,frame=1)
widget_control, state.nav_position_id, set_value=metastrucindex+1
tmp_string = ' of '+strtrim(n_elements(metastruc),2)+'     '
widget_control, state.nav_position_of_id, set_value=strmid(tmp_string,0,10)
nav_button_id = cw_bgroup(nav_base_id, ['Show',' < ',' > '],uvalue='nav_button', /ROW, SPACE=2,ysize=28)

message_base_id = widget_base(sys_base, /base_align_right)
state.messagebar_id = widget_label(buttonbar_base, value = '', xsize='590',ysize='25',frame = 1)
button_base = widget_base(buttonbar_base, /row, /base_align_right)
...
capture_button = widget_button(button_base, $
                            value = 'Capture', $
                            uvalue = 'capture')                            
...
widget_control, top_menu, event_pro = 'acsproc_atv_topmenu_event'
widget_control, state.draw_widget_id, event_pro = 'acsproc_atv_draw_color_event'
widget_control, state.draw_base_id, event_pro = 'acsproc_atv_draw_base_event'
widget_control, state.keyboard_text_id, event_pro = 'acsproc_atv_keyboard_event'
widget_control, state.pan_widget_id, event_pro = 'acsproc_atv_pan_event'
...
; See the routine acsproc_atv_stretchct to see why we do it this way.
...
acsproc_atv_resetwindow

acsproc_atv_colorbar
...
pro acsproc_atv_colorbar

; Routine to tv the colorbar at the bottom of the acsproc_atv window

common acsproc_atv_state

acsproc_atv_setwindow, state.colorbar_window_id
...
pro acsproc_atv_shutdown, windowid

; routine to kill the acsproc_atv window(s) and clear variables to conserve
; memory when quitting acsproc_atv.  The windowid parameter is used when
; acsproc_atv_shutdown is called automatically by the xmanager, if acsproc_atv is
; killed by the window manager.

common acsproc_atv_images
common acsproc_atv_state
common acsproc_atv_color
common acsproc_atv_pdata

; Kill top-level base if it still exists
if (xregistered ('acsproc_atv')) then widget_control, state.base_id, /destroy

; Destroy all pointers to plots and their heap variables
if (nplot GT 0) then begin
    acsproc_atverase, /norefresh
endif
...
;--------------------------------------------------------------------
;                  main acsproc_atv event loops
;--------------------------------------------------------------------

pro acsproc_atv_topmenu_event, event
...
; of acsproc_atv window without entering the main base
acsproc_atv_getwindow
...
; File menu options:
    'ReadFits': begin
        acsproc_atv_readfits, newimage=newimage
        if (newimage EQ 1) then begin
            acsproc_atv_getstats
            acsproc_atv_settitle
            state.zoom_level =  0
            state.zoom_factor = 1.0
            if (state.default_autoscale EQ 1) then acsproc_atv_autoscale
            acsproc_atv_set_minmax
            acsproc_atv_displayall
        endif
    end
    'WritePS' : acsproc_atv_writeps
    'WriteTiff': acsproc_atv_writetiff
    'Quit':     acsproc_atv_shutdown
; ColorMap menu options:            
    'Grayscale': acsproc_atv_getct, 0
    'Blue-White': acsproc_atv_getct, 1
    'Red-Orange': acsproc_atv_getct, 3
    'BGRY': acsproc_atv_getct, 4
    'Rainbow': acsproc_atv_getct, 13
    'Stern Special': acsproc_atv_getct, 15
    'Green-White': acsproc_atv_getct, 8
    'ATV Special': acsproc_atv_makect, event_name
; Scaling options:
    'Linear': begin
        state.scaling = 0
        acsproc_atv_displayall
...
; Label options:
    'TextLabel': acsproc_atv_textlabel
    'Contour': acsproc_atv_oplotcontour
    'Compass': acsproc_atv_setcompass
    'ScaleBar': acsproc_atv_setscalebar
    'EraseLast': acsproc_atverase, 1
    'EraseAll': acsproc_atverase
; Blink options:
    'SetBlink1': begin   
        acsproc_atv_setwindow, state.draw_window_id
...
; Info options:
    'Photometry': acsproc_atv_apphot
    'ImageHeader': acsproc_atv_headinfo
    'Statistics': acsproc_atv_showstats
...
; Coordinate system options:
    ...
    'RA,dec (J2000)': BEGIN 
       ...
       acsproc_atv_gettrack             ; refresh coordinate window
    END 
    'RA,dec (B1950)': BEGIN 
       state.display_coord_sys = 'RA--'
    ...
    'Native': BEGIN 
       IF (state.wcstype EQ 'angle') THEN BEGIN 
          ...
          rev = acsproc_atv_fits_ctype_reversed(astr.ctype)
...
; Help options:            
    'ATV Help': acsproc_atv_help
    
    else: print, 'Unknown event in file menu!'
endcase

; Need to test whether acsproc_atv is still alive, since the quit option
; might have been selected.        
if (xregistered('acsproc_atv', /noshow)) then acsproc_atv_resetwindow

end

;--------------------------------------------------------------------

pro acsproc_atv_locate_event, event
...
if (event.type EQ 0) then begin 
    case event.press of
        1: acsproc_atv_max, /search
        2: acsproc_atv_max, /search
        4: acsproc_atv_max, /search
    endcase
    acsproc_atv_reset_coord
endif

if (event.type EQ 2) then acsproc_atv_draw_motion_event, event

widget_control, state.keyboard_text_id, /sensitive, /input_focus

end
;--------------------------------------------------------------------
pro acsproc_atv_mouse_event, event, click=click, release=release, move=move, alt=alt, command=command, shift=shift, control=control
  ...
  case event.type of
   ...
   endcase
  case event.press of
    2: alt=1L                  ; unfortunately alt-command is interpreted only as alt!
    4: command=1L
    else: alt=0L
   endcase
  case event.modifiers of
    1: shift=1
    2: control=1L
    3: begin                   ; forturnately shift-control is correctly interpreted
      shift=1L 
      control=1L
    end
    else: shift=0L
...
;--------------------------------------------------------------------
pro acsproc_atv_mask_event, event

  ; Event handler for mask mode
  ...
  acsproc_atv_mouse_event, event, click=click, release=release, move=move, alt=alt, command=command, shift=shift, control=control
      
   if (control and click) then begin  ; toggle through the maskmodes, potentially resetting gui for a color event (convenience feature) 
      if (state.maskmode eq 0) then begin
        if alt then acsproc_atv_maskmode, 3, /newmode else acsproc_atv_maskmode, 1, /newmode
      endif else if (state.maskmode eq 1) then begin
        if alt then acsproc_atv_maskmode, 0, /newmode, /colormode else acsproc_atv_maskmode, 2, /newmode
      endif else if (state.maskmode eq 2) then begin
        if alt then acsproc_atv_maskmode, 1, /newmode else acsproc_atv_maskmode, 3, /newmode, /colormode
      endif else if (state.maskmode eq 3) then begin
        if alt then acsproc_atv_maskmode, 2, /newmode else acsproc_atv_maskmode, 0, /newmode
      endif
  endif else begin
    ...
    if click then begin
      state.maskpix = 1L
      maskstruc.saved = 0L
      maskstruc.has_modim = 0L
      maskstruc.has_bizzle = 0L
      maskstruc.has_photmod = 0L
    endif else if release then begin
      state.maskpix = 0L
      maskstruc.saved = 0L
      maskstruc.has_modim = 0L
      maskstruc.has_bizzle = 0L
      maskstruc.has_photmod = 0L
    endif else if (move and state.maskpix eq 1L) then begin
      acsproc_atv_draw_motion_event, event
      acsproc_atv_maskpix, unmask=shift
    endif
...
pro acsproc_atv_draw_color_event, event
  ...
  if click and control then begin ; swap with a locate event (convenience feature) but don't reset gui
    acsproc_atv_locate_event, event
  endif else if (command and click) then begin  ; reset gui for a mask event (convenience feature) 
      if (state.maskmode eq 0) then begin
        if alt then acsproc_atv_maskmode, 3, /newmode else acsproc_atv_maskmode, 1, /newmode, /maskmode
      endif else if (state.maskmode eq 3) then begin
        if alt then acsproc_atv_maskmode, 2, /newmode, /maskmode else acsproc_atv_maskmode, 0, /newmode
      endif else if alt then acsproc_atv_maskmode, state.maskmode-1, /newmode, /maskmode else acsproc_atv_maskmode, state.maskmode+1, /newmode, /maskmode
  endif else if click then begin
      state.cstretch = 1
      acsproc_atv_stretchct, event.x, event.y, /getmouse
      acsproc_atv_colorbar
  endif else if release then begin
    state.cstretch = 0  ; button release
    if (state.bitdepth EQ 24) then acsproc_atv_refresh
    acsproc_atv_draw_motion_event, event
  endif else if move then begin
    if (state.cstretch EQ 1) then begin
      acsproc_atv_stretchct, event.x, event.y, /getmouse 
      if (state.bitdepth EQ 24) then acsproc_atv_refresh, /fast
    endif else begin 
      acsproc_atv_draw_motion_event, event
    endelse
  endif
...
pro acsproc_atv_draw_zoom_event, event
...
if (event.type EQ 0) then begin 
    case event.press of
        1: acsproc_atv_zoom, 'in', /recenter
        2: acsproc_atv_zoom, 'none', /recenter
        4: acsproc_atv_zoom, 'out', /recenter
    endcase
endif
...
pro acsproc_atv_draw_blink_event, event
...
case event.type of
    ...
    2: acsproc_atv_draw_motion_event, event ; motion event
endcase
...
pro acsproc_atv_draw_phot_event, event
...
if (event.type EQ 0) then begin
    case event.press of
        1: acsproc_atv_apphot
        2: acsproc_atv_zoom, 'none', /recenter
        4: acsproc_atv_showstats
        else: 
    endcase
endif
...
pro acsproc_atv_draw_motion_event, event
...
acsproc_atv_gettrack

end

;--------------------------------------------------------------------

pro acsproc_atv_draw_base_event, event
...
pro acsproc_atv_keyboard_event, event
...
case eventchar of
    '1': acsproc_atv_move_cursor, eventchar
    '2': acsproc_atv_move_cursor, eventchar
    '3': acsproc_atv_move_cursor, eventchar
    '4': acsproc_atv_move_cursor, eventchar
    '6': acsproc_atv_move_cursor, eventchar
    '7': acsproc_atv_move_cursor, eventchar
    '8': acsproc_atv_move_cursor, eventchar
    '9': acsproc_atv_move_cursor, eventchar
    'r': acsproc_atv_rowplot
    'R': acsproc_atv_rowplot, /overplot
    'c': acsproc_atv_colplot
    'C': acsproc_atv_colplot, /overplot
    's': acsproc_atv_surfplot
    ;'m': acsproc_atv_markpoint
    'd': acsproc_atv_displaypoint
    'k': acsproc_atv_killpoint
    't': acsproc_atv_contourplot
    'p': acsproc_atv_apphot
    'i': acsproc_atv_showstats
    'q': acsproc_atv_shutdown
    ; ASB added the following keyboard events for mask functions:
    'm': acsproc_atv_maskpix 
    'u': acsproc_atv_maskpix, /unmask
    '.': acsproc_atv_maskreg, /setcorner
    '/': acsproc_atv_maskreg
    '\': acsproc_atv_maskreg, /unmask
    else:  ;any other key press does nothing
endcase

if (xregistered('acsproc_atv', /noshow)) then $
...
pro acsproc_atv_pan_event, event
...
case event.type of
    ...
    2: begin
        acsproc_atv_pantrack, event     ; motion event
        widget_control, state.pan_widget_id, /clear_events
    end
    else:
endcase
...
pro acsproc_atv_event, event



; Main event loop for acsproc_atv top-level base, and for all the buttons.
...
case uvalue of
    ...
    'resize_window': acsproc_atv_resize, setting=event.value+1
    
    'lens_status': acsproc_atv_lens_status, event.index

    'limit_data': begin
      if (event.value) then state.limit_data = event.select
      acsproc_atv_metastruc_limit
    end
    
    'limit_exposures': begin
      if (event.value) then state.limit_exposures = event.select
      acsproc_atv_metastruc_limit
      acsproc_limit_exposures, state.limit_exposures
     end
     
    'nav_list': begin
      acsproc_atv_nav, event.index, /update_position
    end
    
    'nav_position': begin
      widget_control, state.nav_position_id, get_value=navindex
      if navindex ge 1 and navindex lt n_elements(metastruc) then begin
        acsproc_atv_nav, navindex-1, /update_list
      endif else widget_control, state.nav_position_id, set_value=metastrucindex+1
    end
    ...
    'nav_button': begin
      case event.value of
        0: begin ; GET Button
           widget_control, state.nav_position_id, get_value=navindex
           if navindex ge 1 and navindex lt n_elements(metastruc) then metastrucindex = navindex-1 $
           else widget_control, state.nav_position_id, set_value=metastrucindex+1
         end
        1: begin ; < Button
          if metastrucindex gt 0 then metastrucindex -= 1
          widget_control, state.nav_position_id, set_value=metastrucindex+1
        end
        2: begin ; > Button
          if metastrucindex lt n_elements(metastruc)-1 then metastrucindex += 1
          widget_control, state.nav_position_id, set_value=metastrucindex+1
        end
      endcase
      acsproc_atv_nav, metastrucindex, /update_list      
    end
     
    'save': begin
      if state.cutout then acsproc_atv_set_lcoord
      if state.combine then acsproc_atv_savemask
      ;if (state.capture_set and state.capturemode gt 0) then acsproc_atv_capture
      acsproc_set_metadata, /refresh
    end
    
    'brush_size': state.brush_size = ([1,2,3,4,5,10])[event.index] 
    
;    'capture_set': begin
;      state.capture_set = event.select
;      if event.select eq 0 then widget_control, state.capture_droplist_id, set_droplist_select=0L $
;      else widget_control, state.capture_droplist_id, set_droplist_select=state.capturemode                                 
;    end
    
    'mode': acsproc_atv_mode, event.index
  
    'capturemode': state.capturemode = event.index
    
    'imagemode': case event.index of
        0: acsproc_atv_imagemode, /cutout
        1: acsproc_atv_imagemode, /combine
        2: acsproc_atv_imagemode, /combine, /modim
        else: print, 'Unknown Image Mode!'
    endcase
    
    'minusmodim': begin
      if event.index eq 0 then acsproc_atv_messagebar, msgtype='clear'
      state.minusmodimID = event.index-1
      widget_Control, state.minusmodim_fit_id, sensitive=(state.minusmodimID eq 2)
      state.new_minusmodim = 1L
      acsproc_atv_nav, metastrucindex      
    end
    
    'minusmodim_fit': begin
      widget_Control, state.minusmodim_fit_id, get_value=binary_vector
      binary = indgen(n_elements(binary_vector))
      state.maskstruc_nthetaID = fix(total(binary_vector*(2^binary)))
      state.new_minusmodim = 1L
      acsproc_atv_nav, metastrucindex      
    end
    
    'maskmode': acsproc_atv_maskmode, event.index
    ...
    'invert': begin                  ; invert the color table
        ...
        acsproc_atv_stretchct, state.brightness, state.contrast
        if (state.bitdepth EQ 24) then acsproc_atv_refresh
    end
    
    'restretch_button': acsproc_atv_restretch

    'min_text': begin     ; text entry in 'min = ' box
        acsproc_atv_get_minmax, uvalue, event.value
        acsproc_atv_displayall
    end
    ...
    'zoom_in':  acsproc_atv_zoom, 'in'         ; zoom buttons
    'zoom_out': acsproc_atv_zoom, 'out'
    'zoom_one': acsproc_atv_zoom, 'one'
...
pro acsproc_atv_reset_coord

  common acsproc_atv_state
  
        state.coord = state.lcoord
        if state.cutout then acsproc_atv_zoom, 'none', /recenter
  
end
;--------------------------------------------------------------------

pro acsproc_atv_reset_colorscale
  ...
  common acsproc_atv_state
        ...
        acsproc_atv_set_minmax
        if state.cutout then acsproc_atv_autoscale
        acsproc_atv_displayall

end
;--------------------------------------------------------------------

pro acsproc_atv_limit_data, limit_data

; event procedure for moving the box around in the pan window

  common acsproc_atv_state
  
  state.limit_data = limit_data
  widget_control, state.limit_data_id, set_value=state.limit_data
  acsproc_atv_metastruc_limit
...
pro acsproc_atv_limit_exposures, limit_exposures

; event procedure for toggling the limit exposures radio checkbox from parent
...
pro acsproc_atv_messagebar, msg_txt, msgtype=msgtype

; Routine to display an error or warning message.  Message is
; displayed in the messagebar widget_label

  common acsproc_atv_state

  if keyword_set(msgtype) then begin
    if msgtype ne 'clear' then message= strupcase(strcompress(msgtype)) + ': ' else message = ''
  endif
  if n_elements(msg_txt) eq 1 then message += strcompress(msg_txt)
  
  widget_control, state.messagebar_id, set_value=message

end

;----------------------------------------------------------------------

pro acsproc_atv_message, msg_txt, msgtype=msgtype, window=window
...
;-----------------------------------------------------------------------
;      main acsproc_atv routines for scaling, displaying, cursor tracking...
;-----------------------------------------------------------------------

pro acsproc_atv_displayall, first=first
...
; passed to acsproc_atv.  If the display image has just been moved around or
; zoomed without a change in scaling, then just call acsproc_atv_refresh
; rather than this routine.

acsproc_atv_scaleimage
acsproc_atv_makepan
acsproc_atv_refresh
...
pro acsproc_atv_refresh, fast = fast
...
if (not(keyword_set(fast))) then begin
    acsproc_atv_getoffset
    acsproc_atv_getdisplay
    acsproc_atv_displaymain
    acsproc_atv_plotall
endif else begin
    acsproc_atv_displaymain
endelse
...
acsproc_atv_setwindow, state.pan_pixmap
erase
tv, pan_image, state.pan_offset[0], state.pan_offset[1]
acsproc_atv_resetwindow

acsproc_atv_setwindow, state.pan_window_id
if (not(keyword_set(fast))) then erase
tv, pan_image, state.pan_offset[0], state.pan_offset[1]
acsproc_atv_resetwindow
acsproc_atv_drawbox, /norefresh

if (state.bitdepth EQ 24) then acsproc_atv_colorbar

; redisplay the tracking image
if (not(keyword_set(fast))) then acsproc_atv_gettrack
...
pro acsproc_atv_move_cursor, direction
...
pro acsproc_atv_get_minmax, uvalue, newvalue
...
pro acsproc_atv_zoom, zchange, recenter = recenter
common acsproc_atv_state
...
case zchange of
    ...
    else:  print,  'problem in acsproc_atv_zoom!'
endcase
...
;-----------------------------------------------------------------------
pro acsproc_atv_max, search = search
common acsproc_atv_state
common acsproc_atv_images

; Routine to set location coord at cross-hair location
; or if keyword search is set, at the value max in 
; the vicinity (+-5 pixels) of the  cross-hair location

if keyword_set(search) then begin
  delta_search = 8
  search_image_size = 2*delta_search + 1
  search_image = main_image[state.coord[0]-delta_search:state.coord[0]+delta_search,state.coord[1]-delta_search:state.coord[1]+delta_search]
  search_image_min = min(search_image, max=search_image_max, /nan)
  wh = where(search_image eq search_image_max,c)
  if c gt 0 then state.lcoord = [state.coord[0]+(wh[0] mod search_image_size)-delta_search,state.coord[1]+(wh[0] / search_image_size)-delta_search]
endif else state.lcoord = state.coord
widget_control,state.cutout_locatebar_id, set_value = 'Location '+string(state.lcoord[0],state.lcoord[1], format = '("(",i3,",",i3,") ")' )
end

;-----------------------------------------------------------------------

pro acsproc_atv_autoscale
...
function acsproc_atv_fits_ctype_reversed, ctype
...
function acsproc_atv_wcsstring, lon, lat, ctype, equinox, disp_type, disp_equinox, $
            disp_base60
...
; Check for reversed (RA,dec) etc. 
  reverse = acsproc_atv_fits_ctype_reversed(ctype)
...
function acsproc_atv_wavestring
...
acsproc_atv_setwindow, state.track_window_id
tv, track_image
...
if (state.wcstype EQ 'angle') then begin
    ...
    wcsstring = acsproc_atv_wcsstring(lon, lat, (*state.astr_ptr).ctype,  $
...
if (state.wcstype EQ 'lambda') then begin
    wavestring = acsproc_atv_wavestring()
...
pro acsproc_atv_drawbox, norefresh=norefresh
...
pro acsproc_atv_pantrack, event
...
pro acsproc_atv_resize, resize=resize, setting=setting

; Routine to resize the draw window when a top-level resize event
; occurs.  Completely overhauled by AB (and JRB) for v1.4. 

  common acsproc_atv_state
  common acsproc_atv_data 
  
  
  if keyword_set(setting) then begin
    if setting le 3 then begin
      newsize = ([4.0,4.33,6.0] * pixels_per_arcsec)[setting-1]
      resize = [newsize,newsize]
    endif else resize = [8.2,4.3] * pixels_per_arcsec
  endif
  ...
  if keyword_set(resize) then begin
    ...
    newxsize = (tmp_event[0] - state.base_pad[0]) > (state.base_min_size[0] - state.base_pad[0]) 
    newysize = (tmp_event[1] - state.base_pad[1]) > (state.base_min_size[1] - state.base_pad[1])
  endelse
  ...
  if keyword_set(resize) then acsproc_atv_refresh

end

;----------------------------------------------------------------------

pro acsproc_atv_scaleimage
...
pro acsproc_atv_getstats, align=align
...
; acsproc_atv_images common block
...
pro acsproc_atv_setwindow, windowid
...
; (i.e. non-acsproc_atv) idl window.  Use acsproc_atv_setwindow to set the window to
; one of the acsproc_atv window, then display something to that window, then
; use acsproc_atv_resetwindow to set the current window back to the currently
...
; graphics to the previous acsproc_atv window actually get displayed to screen
...
pro acsproc_atv_readfits, fitsfilename=fitsfilename, newimage=newimage
...
if (n_elements(strcompress(head, /remove_all)) LT 2) then begin
    acsproc_atv_message, 'File does not appear to be a valid fits image!', $
...
if (numext EQ 0 AND naxis LT 2) then begin
   if ishealpix(npix = sxpar(head, 'NAXIS1'), /silent) EQ 0 then begin 
      acsproc_atv_message, 'Selected file is not a 2-d FITS image!', $
        window = window, msgtype = 'error'
      return
   endif
endif
...
acsproc_atv_fitsext_read, fitsfile, extension, head, cancelled
;if ((numext GT 0) AND (instrume NE 'WFPC2')) then begin
;    acsproc_atv_fitsext_read, fitsfile, numext, head, cancelled
;endif else if ((instrume EQ 'WFPC2') AND (naxis EQ 3)) then begin
;    acsproc_atv_wfpc2_read, fitsfile, head, cancelled
;endif else if ((naxis EQ 3) AND (origin EQ '2MASS')) then begin
;    acsproc_atv_2mass_read, fitsfile, head, cancelled
;endif else begin
;    acsproc_atv_plainfits_read, fitsfile, head, cancelled
;endelse
...
pro acsproc_atv_fitsext_read, fitsfile, numext, head, cancelled
...
; -------- check for healpix bintable (e.g. WMAP format)
   if ishealpix(npix = sxpar(head, 'NAXIS2'), /silent) EQ 0 then begin 
      acsproc_atv_message, 'File appears to be a FITS table, not an image.', $
...
if state.combine then acsproc_atv_loadmask
end

;---------------------------------------------------------------------

function acsproc_atv_photmodstruc, photmodID

  common acsproc_atv_data
  common acsproc_atv_images
  
  photmodstruc = {fitfunc: ''}
  
  if photmodID eq 1 or photmodID eq 2 then begin
    photmodfile = acsproc_datafile(metastruc[metastrucindex],/photmod)
    if file_test(photmodfile) then begin
      photmodstruc = mrdfits(photmodfile, photmodID, /silent)
    endif
  endif
...
pro acsproc_atv_minus_modim
  ...
  if state.minusmodimID ge 0 and state.minusmodimID lt 3 then begin
  
    xc = state.lcoord[0]
    yc = state.lcoord[1]

    maskstruc.modimID = state.minusmodimID
    maskstruc.nthetaID = state.maskstruc_nthetaID
    
    modim = acsproc_modim(/fit_nthetaID)    
      
    if not state.modim then begin
      main_image[xc-hw:xc+hw,yc-hw:yc+hw] -= modim.modim 
    endif else begin
      main_image = main_image*0.0
      main_image[xc-hw:xc+hw,yc-hw:yc+hw] = modim.modim
    endelse
    
    void = where(modim.modim ne 0.,has_modim)
    
    if (has_modim gt 0) then begin
      message = 'chi2/dof = '+strtrim(modim.chi2,2)
      modimtype = modim.name
      if modimtype eq 'Bspline Fit' then modimtype += '('+strtrim(modim.nthetaID,2)+')['+acsproc_ntheta(modim, /str)+']'
      acsproc_atv_messagebar, message, msgtype=modimtype
    endif else acsproc_atv_messagebar, msgtype='clear'
    
  endif else if state.minusmodimID eq 3 or state.minusmodimID eq 4 then begin
     
    photmodID = state.minusmodimID-2
    photmodstruc = acsproc_atv_photmodstruc(photmodID)
    if (photmodstruc.fitfunc ne '') then begin
      if not state.modim then main_image -= photmodstruc.img $
      else main_image = photmodstruc.img
      message = 'chi2/dof = '+strtrim(photmodstruc.chisq/photmodstruc.ndof,2)
      if photmodstruc.fitfunc eq 'asb_devauc_multi' then photmodtype = 'de Vaucouleurs' $
      else if photmodstruc.fitfunc eq 'asb_sersic_multi' then begin
        photmodtype = 'Sersic'
        message = ' [index = '+strtrim(photmodstruc.pars[6],2) + '] ' + message
      endif
      acsproc_atv_messagebar, message, msgtype=photmodtype
    endif
...
pro acsproc_atv_plainfits_read, fitsfile, head, cancelled
...
pro acsproc_atv_wfpc2_read, fitsfile, head, cancelled
...
pro acsproc_atv_2mass_read, fitsfile, head, cancelled
...
fileType = state.capturemodelist[state.capturemode]

; Get filename to save image

if (state.modim) then subext = 'modim' $
else if (state.combine and state.minusmodimID gt 0) then subext='srcim' $
else if state.combine then subext='comb' $
else if state.cutout then subext = 'cutout' $
else subext=''
filename=acsproc_datafile(metastruc[metastrucindex], combine=state.combine, cutout=state.cutout, subext=subext, ext=fileType)

if (filename eq '') then begin
  filename = dialog_pickfile(filter = '*.'+fileType, $ 
                           file = metastruc[metastrucindex].rootname, $
                           group =  state.base_id, $
                           path = state.current_dir, $
                           get_path = tmp_dir, $
                           /write)
...
if (filename EQ state.current_dir) then begin
    acsproc_atv_message, 'Must indicate filename to save.', msgtype = 'error', /window
    return
endif
...
fname = strcompress(state.current_dir + 'acsproc_atv.ps', /remove_all)
...
pro acsproc_atv_stretchct, brightness, contrast,  getmouse = getmouse
...
pro acsproc_atv_getct, tablenum
...
function acsproc_atv_polycolor, p

; Routine to return an vector of length !d.table_size-8,
; defined by a 5th order polynomial.   Called by acsproc_atv_makect
...
pro acsproc_atv_makect, tablename
...
case tablename of
    'ATV Special': begin
        r = acsproc_atv_polycolor([39.4609, $
                           -5.19434, $
                           0.128174, $
                           -0.000857115, $
                           2.23517e-06, $
        ...
        g = acsproc_atv_polycolor([-15.3496, $
                           1.76843, $
                           -0.0418186, $
                           0.000308216, $
                           -6.07106e-07, $
...
function acsproc_atv_icolor, color
...
pro acsproc_atv_settitle, row=row

if not keyword_set(row) then row=-1

common acsproc_atv_state

; Update title bar with the image file name

  widget_control, state.base_id, tlb_set_title = acsproc_atv_gettitle(row=row)

end

;----------------------------------------------------------------------

pro acsproc_atv_setheader, head


; Routine to keep the image header using a pointer to a 
; heap variable.  If there is no header (i.e. if acsproc_atv has just been
...
if (xregistered('acsproc_atv_headinfo')) then begin
    widget_control, state.headinfo_base_id, /destroy
endif

if (xregistered('acsproc_atv_stats')) then begin
...
if (not(ptr_valid(state.head_ptr))) then begin
    ...
    acsproc_atv_message, 'No header information available for this image!', $
      msgtype = 'error', /window
    return
endif
...
if (not(xregistered('acsproc_atv_headinfo', /noshow))) then begin

    headinfo_base = $
      widget_base(/base_align_right, $
                  group_leader = state.base_id, $
                  /column, $
                  title = 'acsproc_atv image header information', $
    ...
    xmanager, 'acsproc_atv_headinfo', headinfo_base, /no_block
...
pro acsproc_atv_headinfo_event, event
...
pro acsproc_atv_plot1plot, iplot
common acsproc_atv_pdata
common acsproc_atv_state
...
pro acsproc_atv_plot1text, iplot
common acsproc_atv_pdata
common acsproc_atv_state
...
pro acsproc_atv_plot1contour, iplot
common acsproc_atv_pdata
common acsproc_atv_state
...
pro acsproc_atv_plot1compass, iplot
...
pro acsproc_atv_plot1scalebar, iplot
...
acsproc_atv_arcbar, *(state.head_ptr), $
...
pro acsproc_atv_arcbar, hdr, arclen, LABEL = label, SIZE = size, THICK = thick, $
...
; abbreviated for acsproc_atv and modified to work with zoomed images.  For
...
acsproc_atv_plotwindow

for iplot = 1, nplot do begin
    case (*(plot_ptr[iplot])).type of
        'points'  : acsproc_atv_plot1plot, iplot
        'text'    : acsproc_atv_plot1text, iplot
        'contour' : acsproc_atv_plot1contour, iplot
        'compass' : acsproc_atv_plot1compass, iplot
        'scalebar': acsproc_atv_plot1scalebar, iplot
        else      : print, 'Problem in acsproc_atv_plotall!'   
    endcase
endfor
...
pro acsproc_atvplot, x, y, _extra = options
common acsproc_atv_pdata
common acsproc_atv_state
...
if (not(xregistered('acsproc_atv', /noshow))) then begin
...
;  convert color names to index numbers, and set default=red
   ...
   options.color = acsproc_atv_icolor(options.color)
...
pro acsproc_atvxyouts, x, y, text, _extra = options
common acsproc_atv_pdata
common acsproc_atv_state
...
pro acsproc_atvcontour, z, x, y, _extra = options
common acsproc_atv_pdata
common acsproc_atv_state
...
; need not be the same dataset displayed in the acsproc_atv window, but it
...
;  convert color names to index numbers, and set default=red
   ...
   options.c_color = acsproc_atv_icolor(options.c_color)
   ...
   acsproc_atv_plotwindow
   acsproc_atv_plot1contour, nplot
...
pro acsproc_atverase, nerase, norefresh = norefresh
common acsproc_atv_pdata
...
if (NOT keyword_set(norefresh)) then acsproc_atv_refresh

end

;----------------------------------------------------------------------

pro acsproc_atv_textlabel

; widget front end for acsproc_atvxyouts
...
; switch red and black indices
    ...
    acsproc_atvxyouts, textform.tag1, textform.tag2, textform.tag0, $
...
pro acsproc_atv_oplotcontour

; widget front end for acsproc_atvcontour
...
; switch red and black indices
    ...
    acsproc_atvcontour, main_image, c_color = labelcolor, $
...
if (nplot GE maxplot) then begin
    acsproc_atv_message, 'Total allowed number of overplots exceeded.', $
      msgtype = 'error', /window
    return
endif
...
if (state.wcstype NE 'angle') then begin 
    acsproc_atv_message, 'Cannot get coordinate info for this image!', $
      msgtype = 'error', /window
    return
endif
...
cform = cw_form(formdesc, /column, $
                   title = 'acsproc_atv compass properties')
...
acsproc_atv_plotwindow
acsproc_atv_plot1compass, nplot

end

;---------------------------------------------------------------------

pro acsproc_atv_setscalebar
...
cform = cw_form(formdesc, /column, $
                   title = 'acsproc_atv scalebar properties')
...
acsproc_atv_plotwindow
acsproc_atv_plot1scalebar, nplot
...
xmanager, 'acsproc_atv_lineplot', state.lineplot_base_id, /no_block
...
pro acsproc_atv_rowplot, overplot=overplot
...
pro acsproc_atv_colplot, overplot=overplot
...
pro acsproc_atv_surfplot

common acsproc_atv_state
common acsproc_atv_images

if (not (xregistered('acsproc_atv_lineplot', /noshow))) then begin
    acsproc_atv_lineplot_init
endif

acsproc_atv_setwindow, state.lineplot_window_id
erase
...
pro acsproc_atv_markpoint

common acsproc_atv_state
common acsproc_atv_point
...
acsproc_atverase
acsproc_atvplot, markcoord[0,*], markcoord[1,*], psym=4

end

;--------------------------------------------------------------------

pro acsproc_atv_displaypoint
...
pro acsproc_atv_lineplot_event, event
...
i = i + 1
h[i] =  'File->Quit:             Quits acsproc_atv'
i = i + 1
...
i = i + 1
h[i] = '    q: quits acsproc_atv'
i = i + 2
h[i] = 'IDL COMMAND LINE HELP:'
i = i + 1
h[i] =  'To pass an array to acsproc_atv:'
i = i + 1
h[i] =  '   acsproc_atv, array_name [, options]'
i = i + 1
h[i] = 'To pass a fits filename to acsproc_atv:'
i = i + 1
h[i] = '    acsproc_atv, fitsfile_name [, options] (enclose filename in single quotes) '
i = i + 1
...
i = i + 1
h[i] = '    acsproc_atvcontour, array_name [, options...]'
i = i + 1
h[i] = 'To overplot text on the draw window: '
i = i + 1
h[i] = '    acsproc_atvxyouts, x, y, text_string [, options]  (enclose string in single quotes)'
i = i + 1
h[i] = 'To overplot points or lines on the current plot:'
i = i + 1
h[i] = '    acsproc_atvplot, xvector, yvector [, options]'
i = i + 2
h[i] = 'The options for acsproc_atvcontour, acsproc_atvxyouts, and acsproc_atvplot are essentially'
i = i + 1
...
h[i] = 'Other commands:'
i = i + 1
h[i] = 'acsproc_atverase [, N]:       erases all (or last N) plots and text'
i = i + 1
h[i] = 'acsproc_atv_shutdown:   quits acsproc_atv'
i = i + 1
h[i] = 'NOTE: If acsproc_atv should crash, type acsproc_atv_shutdown at the idl prompt.'
i = i + 5
h[i] = strcompress('ATV.PRO version '+state.version)
i = i + 1
h[i] = 'For full instructions, or to download the most recent version, go to:'
i = i + 1
h[i] = 'http://www.astro.caltech.edu/~barth/acsproc_atv/acsproc_atv.html'


if (not (xregistered('acsproc_atv_help', /noshow))) then begin

helptitle = strcompress('acsproc_atv v' + state.version + ' help')
    ...
    xmanager, 'acsproc_atv_help', help_base, /no_block
...
pro acsproc_atv_help_event, event
...
pro acsproc_atv_stats_event, event
...
if (not (xregistered('acsproc_atv_stats', /noshow))) then begin

    stats_base = $
      widget_base(group_leader = state.base_id, $
                  /column, $
                  /base_align_center, $
                  title = 'acsproc_atv image statistics', $
                  uvalue = 'stats_base')
    ...
    xmanager, 'acsproc_atv_stats', stats_base, /no_block
...
;---------------------------------------------------------------------
pro acsproc_atv_maskreg, setcorner=setcorner, unmask=unmask

; ASB routine to interactively mask regions.
; Works with same mask array as "acsproc_atv_maskpix".
; Press "." to define one corner of the mask region,
; press "/" to mask to the other corner, and press "\"
; to perform unmasking action instead of masking.

  common acsproc_atv_state
  common acsproc_maskstruc
  
  if state.combine then begin
  
    mask_coord = state.coord
    if keyword_set(setcorner) then cornerpix=[mask_coord[0], mask_coord[1]] else begin
      xlo = min([mask_coord[0], cornerpix[0]])
      xhi = max([mask_coord[0], cornerpix[0]])
      ylo = min([mask_coord[1], cornerpix[1]])
      yhi = max([mask_coord[1], cornerpix[1]])
      case state.maskmode of
        1: maskstruc.fmask[xlo:xhi,ylo:yhi] = keyword_set(unmask) ? 0B : 1B
        2: maskstruc.jmask[xlo:xhi,ylo:yhi] = keyword_set(unmask) ? 0B : 1B
        else: return
      endcase

      
      acsproc_atv_showmask
    endelse
...
pro acsproc_atv_maskpix, unmask=unmask

; ASB routine to interactively tag individual pixels in
; an image.  Maintains a mask array called ["maskstruc.fmask","maskstruc.jmask"]
; depending on the state.maskmode = [1,2], respectively. 
; the common block "acsproc_maskstruc" (note underscore).  Use
; function acsproc_atv_getmask() to access the mask. 
  ...
  if state.combine then begin
    ...
    if (state.brush_size mod 2) then brush0 = state.brush_size/2 $
    else brush0 = (state.brush_size-1)/2
    brush1 = state.brush_size/2
    
    case state.maskmode of
      1: maskstruc.fmask[x-brush0:x+brush1,y-brush1:y+brush0] = keyword_set(unmask) ? 0B : 1B
      2: maskstruc.jmask[x-brush0:x+brush1,y-brush1:y+brush0] = keyword_set(unmask) ? 0B : 1B
      else: return
    endcase
    acsproc_atv_showmask
...
pro acsproc_atv_showmask
  ...
  common acsproc_data 
    ...
    case state.maskmode of
        0: return
        1: acsproc_atv_plotmask, maskstruc.fmask, color='red'
        2: acsproc_atv_plotmask, maskstruc.jmask, color='green'
        3: begin
          acsproc_atv_plotmask, maskstruc.fmask, color='red'
          acsproc_atv_plotmask, maskstruc.jmask, color='green'
        end
    endcase
    ...
    ;plot blue square around mask region
      
    acsproc_atvplot, [xc-hw,xc+hw],[yc-hw,yc-hw],color='blue'
    acsproc_atvplot, [xc-hw,xc-hw],[yc-hw,yc+hw],color='blue'
    acsproc_atvplot, [xc-hw,xc+hw],[yc+hw,yc+hw],color='blue'
    acsproc_atvplot, [xc+hw,xc+hw],[yc-hw,yc+hw],color='blue'
...
pro acsproc_atv_plotmask, mask, ps=ps, color=color

  common acsproc_atv_state
  
    if not keyword_set(ps) then ps=6
  
    nc = state.image_size[0]
    mpix = where(mask, nwhere)
    if (nwhere gt 0) then acsproc_atvplot, mpix mod nc, mpix / nc, ps=ps, color=color
    
end

;----------------------------------------------------------------------

function acsproc_atv_getmask, feature=feature, junk=junk

; ASB routine to dump interactive pixel mask.

  common acsproc_maskstruc
  common acsproc_data
  
  mask = bytarr(imsize,imsize)
  if keyword_set(feature) then mask += maskstuc.fmask
  if keyword_set(junk) then mask += maskstruc.jmask
...
pro acsproc_atv_savemask

; ASB routine to set interactive pixel mask.

  common acsproc_atv_data
  common acsproc_maskstruc
  common acsproc_atv_state
     
    maskfile = acsproc_datafile(metastruc[metastrucindex],/mask)    
    photmodfile = acsproc_datafile(metastruc[metastrucindex],/photmod)   
    if maskstruc.has_photmod then if not file_test(photmodfile) then maskstruc.has_photmod = 0L
         
    if maskstruc.saved then begin
      if maskstruc.has_bizzle then maskflag = metastruc[metastrucindex].maskflag else maskflag = 2
    endif else begin
      if file_test(maskfile) then begin
        if maskstruc.has_photmod then maskflag = 4 $
        else if maskstruc.has_bizzle then maskflag = 3 $
        else if maskstruc.has_modim then maskflag = 2 $
        else maskflag =1
      endif else maskflag = 0
    endelse
    
    if maskflag gt 0 then maskstruc.has_modim = 1L
    acsproc_maskstruc_write, maskfile

    void = where(maskstruc.fmask ne 0, has_fmask)
    fmaskflag = has_fmask gt 0
    void = where(maskstruc.jmask ne 0, has_jmask)
    jmaskflag = has_jmask gt 0

    if (maskflag ne metastruc[metastrucindex].maskflag) or (fmaskflag ne metastruc[metastrucindex].fmaskflag) or (jmaskflag ne metastruc[metastrucindex].jmaskflag) then begin
      metastruc[metastrucindex].maskflag = maskflag
      metastruc[metastrucindex].fmaskflag=fmaskflag
      metastruc[metastrucindex].jmaskflag=jmaskflag
      acsproc_update_maskflag, metastruc[metastrucindex]
    endif
...
; ASB routine to set interactive pixel mask.
  ...
  common acsproc_maskstruc
    ...
    if not state.new_minusmodim then begin
      if state.minusmodimID lt 3 then begin 
        if state.minusmodimID ge 0 then begin 
          state.minusmodimID = maskstruc.modimID
          widget_Control, state.minusmodim_id, set_droplist_select=state.minusmodimID+1
        endif
        state.maskstruc_nthetaID = maskstruc.nthetaID
        binary_vector = [0,0,0]
        for v=0,n_elements(binary_vector)-1 do begin
          if ((maskstruc.nthetaID and 2^v) eq 2^v) then binary_vector[v] = 1
        endfor
        widget_Control, state.minusmodim_fit_id, set_value=binary_vector        
      endif
    endif else begin
      if maskstruc.has_modim and (state.maskstruc_nthetaID ne maskstruc.nthetaID) then maskstruc.has_modim = 0L
      if maskstruc.has_bizzle and (state.maskstruc_nthetaID ne maskstruc.nthetaID) then maskstruc.has_bizzle = 0L
      state.new_minusmodim = 0L
    endelse
            
    acsproc_atv_minus_modim

    widget_control, state.lens_status_id, set_droplist_select = maskstruc.lens_status
  
end

;---------------------------------------------------------------------

pro acsproc_atv_lens_status, lens_status
...
; ASB routine to set interactive pixel mask.
  ...
  if maskstruc.lens_status ne lens_status then begin
    maskstruc.lens_status = lens_status
    maskfile = acsproc_datafile(metastruc[metastrucindex],/mask)
    acsproc_maskstruc_write, maskfile
    lens = ['0','2','1','3']
    acsproc_set_target_lens, metastruc[metastrucindex].targname, lens[lens_status]
  endif
  acsproc_set_metadata, /refresh
end
...
pro acsproc_atv_mode, index, newmode=newmode

  common acsproc_atv_state

  if keyword_set(newmode) then Widget_Control, state.mode_droplist_id, set_droplist_select=index  
  
  widget_Control, state.brush_size_id, sensitive=(index eq 3)
  
  case index of
      0: widget_control, state.draw_widget_id, $
        event_pro = 'acsproc_atv_draw_color_event'
      1: widget_control, state.draw_widget_id, $
        event_pro = 'acsproc_atv_draw_zoom_event'
      4: widget_control, state.draw_widget_id, $
        event_pro = 'acsproc_atv_draw_blink_event'
      5: widget_control, state.draw_widget_id, $
        event_pro = 'acsproc_atv_draw_phot_event'
      2: widget_control, state.draw_widget_id, $
        event_pro = 'acsproc_atv_locate_event'
      3: widget_control, state.draw_widget_id, event_pro = 'acsproc_atv_mask_event'
...
pro acsproc_atv_imagemode, cutout=cutout, combine=combine, modim=modim, lcoords=lcoords, newmode=newmode

  common acsproc_atv_state

  state.cutout = keyword_set(cutout)
  state.combine = keyword_set(combine)
  state.modim =  keyword_set(modim)
      
  Widget_Control, state.maskmode_droplist_id, Sensitive=keyword_set(combine)
  Widget_Control, state.limit_exposures_id, Sensitive=keyword_set(cutout)
  Widget_Control, state.lens_status_id, Sensitive=keyword_set(combine)
  
  if keyword_set(cutout) then begin
    ;state.capture_set = 1
    ;Widget_Control, state.capture_set_id, set_button=state.capture_set 
    if keyword_set(newmode) then Widget_Control, state.imagemode_droplist_id, set_droplist_select=0 
  endif else if keyword_set(combine) then begin
    ;if state.maskmode eq 0 or state.maskmode eq 3 then state.capture_set = 1 else state.capture_set = 0
    ;Widget_Control, state.capture_set_id, set_button=state.capture_set                                 
    if keyword_set(newmode) then Widget_Control, state.imagemode_droplist_id, set_droplist_select=1                             
    state.limit_exposures = 1L
    widget_control, state.limit_exposures_id, set_value=state.limit_exposures
    acsproc_limit_exposures, state.limit_exposures
  endif

  acsproc_atv_metastruc_limit, lcoords=lcoords
  acsproc_atv_reset_colorscale
...
pro acsproc_atv_maskmode, index, newmode=newmode, colormode=colormode, maskmode=maskmode

  common acsproc_atv_state

  if keyword_set(maskmode) then acsproc_atv_mode, 3, /newmode else if keyword_set(colormode) then acsproc_atv_mode, 0, /newmode
  if keyword_set(newmode) then Widget_Control, state.maskmode_droplist_id, set_droplist_select=index  
  
  if state.combine then begin
     oldmaskmode =  state.maskmode
     state.maskmode = index
;     case index of
;      0: begin
;        state.capture_set = 1
;        Widget_Control, state.capture_set_id, set_button=state.capture_set
...
pro acsproc_atv_nav, index, update_list=update_list, update_position=update_position
  ...
  hw=25
  metastrucindex = index
  if keyword_set(update_position) then widget_control, state.nav_position_id, set_value=metastrucindex+1
  if keyword_set(update_list) then widget_control, state.nav_list_id, set_droplist_select=metastrucindex
  image = acsproc_datafile(metastruc[metastrucindex], cutout=state.cutout, combine=state.combine)
  acsproc_atv_readfits, fitsfilename=image, newimage=newimage
  if (newimage EQ 1) then begin
      acsproc_atv_getstats
      ...
      xc = state.lcoord[0]
      yc = state.lcoord[1]
      state.max_value = max(main_image[xc-hw:xc+hw,yc-hw:yc+hw])
      acsproc_atv_set_minmax
      acsproc_atv_displayall
      if state.combine then acsproc_atv_showmask
...
pro acsproc_atv_metastruc_nav, index

; routine to navigate to metastruc[index] in acsproc_atv
;
  common acsproc_atv_state
  ...
  if state.limit_data then begin
    if state.limit_exposures then begin
      wh = where(metastruc.subexposure eq 1, c)
      if c gt 0 then begin
        visits = metastruc[wh]
      endif else visits = metastruc
    endif else visits = metastruc
    visit = visits[index]   
    wh = where(acsproc_goodones(visits),c)
    if c gt 0 then visits = visits[wh]
    index = where(visits.proposid eq visit.proposid and visits.visit eq visit.visit and visits.exposure eq visit.exposure and visits.subexposure eq 1 and acsproc_goodones(visits), c)
    if (c eq 1) then begin
      acsproc_atv_nav, index, /update_list, /update_position
    endif
    
  endif else acsproc_atv_nav, index, /update_list, /update_position
...
acsproc_atv_setwindow, state.statzoom_window_id
erase
...
pro acsproc_atv_imcenterf, xcen, ycen
...
function acsproc_atv_splinefwhm, rad, prof, splrad, splprof
...
pro acsproc_atv_radplotf, x, y, fwhm
...
fwhm = acsproc_atv_splinefwhm(out[*,0],out[*,6])
...
pro acsproc_atv_apphot_refresh
...
; Run acsproc_atv_radplotf and plot the results

acsproc_atv_setwindow, state.radplot_window_id
acsproc_atv_radplotf, x, y, fwhm
...
; Uncomment next lines if you want acsproc_atv to output the WCS coords of 
; the centroid for the photometry object:
;if (state.wcstype EQ 'angle') then begin
;    xy2ad, state.centerpos[0], state.centerpos[1], *(state.astr_ptr), $
;      clon, clat
;    wcsstring = acsproc_atv_wcsstring(clon, clat, (*state.astr_ptr).ctype,  $
...
acsproc_atv_setwindow, state.photzoom_window_id
erase
...
pro acsproc_atv_apphot_event, event
...
case uvalue of
    ...
    'photsettings': acsproc_atv_apphot_settings
...
pro acsproc_atv_apphot_settings
...
textform = cw_form(formdesc, /column, $
                   title = 'acsproc_atv photometry settings')
...
if (not (xregistered('acsproc_atv_apphot', /noshow))) then begin

    apphot_base = $
      widget_base(/base_align_center, $
                  group_leader = state.base_id, $
                  /column, $
                  title = 'acsproc_atv aperture photometry', $
                  uvalue = 'apphot_base')
    ...
    xmanager, 'acsproc_atv_apphot', apphot_base, /no_block
...
pro acsproc_atv_set_lcoord

common acsproc_atv_data
common acsproc_atv_state
  
  if state.cutout then begin
    sxaddpar, *state.head_ptr, 'LCOORDX', state.lcoord[0], 'manually selected lens coord'
    sxaddpar, *state.head_ptr, 'LCOORDY', state.lcoord[1], 'manually selected lens coord'
    ofile = acsproc_datafile(metastruc[metastrucindex], /cutout)
    modfits, ofile, 0, *state.head_ptr, EXTEN_NO=1
  endif

end

;--------------------------------------------------------------------
pro acsproc_atv_get_lcoord

common acsproc_atv_state

    if state.cutout then lcoord = [fix(sxpar(*state.head_ptr, 'LCOORDX')),fix(sxpar(*state.head_ptr, 'LCOORDY'))] $
    else if state.combine then lcoord = [fix(sxpar(*state.head_ptr, 'CRPIX1'))-1L,fix(sxpar(*state.head_ptr, 'CRPIX2'))-1L] $
    else lcoord = [0,0]
    if (lcoord[0] gt 0 and lcoord[1] gt 0) then state.lcoord=lcoord
    
end

;--------------------------------------------------------------------
;    acsproc_atv main program.  needs to be last in order to compile.
;---------------------------------------------------------------------

; Main program routine for ATV.  If there is no current ATV session,
; then run acsproc_atv_startup to create the widgets.  If ATV already exists,
; then display the new image to the current ATV window.

pro acsproc_atv, image, $
         min = minimum, $
         max = maximum, $
         autoscale = autoscale,  $
         linear = linear, $
         ...
         header = header, $
         mark = mark, $
         dataroot=this_dataroot, cycle=this_cycle, program=this_program, reset=reset
...
if imtype eq 'STRUCT' then begin
  if keyword_set(this_dataroot) then dataroot = this_dataroot else dataroot = '/'
  if keyword_set(this_program) then program = this_program
  if keyword_set(this_cycle) then cycle = this_cycle
  metastruc = image
  wh = where(metastruc.cutflag, c)
  if (c gt 0) then begin
    metastruc = metastruc[wh]
    metastruc0 = metastruc
    if keyword_set(reset) then metastrucindex = 0
    image = acsproc_datafile(metastruc[metastrucindex], /cutout)
    imtype = 'STRING'
  endif else begin
    metastruc0 = metastruc
    if keyword_set(reset) then metastrucindex = 0
    image = acsproc_datafile(metastruc[metastrucindex])
    imtype = 'STRING'
  endelse
endif else metastruc = 0L
...
pixels_per_arcsec = 160.0
newimage = 0

;if ( (n_params() EQ 0) AND (xregistered('acsproc_atv', /noshow))) then begin
;    print, 'USAGE: acsproc_atv, array_name OR fitsfile'
...
; Before starting up acsproc_atv, get the user's external window id.  We can't
; use the acsproc_atv_getwindow routine yet because we haven't run acsproc_atv
; startup.  A subtle issue: acsproc_atv_resetwindow won't work the first time
...
; If user has passed acsproc_atv a data array, read it into main_image.
if ( (n_params() NE 0) AND (imtype NE 'STRING') AND $
...
if (n_elements(main_image) LE 1) then begin
    ...
    imagename = ''
    newimage = 1
    acsproc_atv_setheader, ''
endif

if (newimage EQ 1) then begin  
; skip this part if new image is invalid or if user selected 'cancel'
; in dialog box
    if size(metastruc, /tname) eq 'STRUCT' then acsproc_atv_settitle,row=0 else acsproc_atv_settitle
    acsproc_atv_getstats, align=align
...
; not set
    ...
    state.centerpix=state.lcoord
    state.zoom_level=3
    state.zoom_factor = 2.^state.zoom_level
    
    state.invert_colormap = 1L ; invert
    acsproc_atv_getct, 0 ; grayscale
    
   ; acsproc_atv_getct, 15
    acsproc_atv_getoffset
    acsproc_atv_refresh
...
if (not(xregistered('acsproc_atv', /noshow))) then begin
    nb = abs(block - 1)
    xmanager, 'acsproc_atv', state.base_id, no_block = nb, cleanup = 'acsproc_atv_shutdown'
    wset, userwindow
    ; if blocking mode is set, then when the procedure reaches this
    ; line acsproc_atv has already been terminated.  If non-blocking, then
