  ...
  if (nvis gt 0) then begin
    ...
    for q = 0,nvis-1 do begin
      ...
      if keyword_set(smartguess) then begin
          ...
          epsilon = 1.0e-6
          epsilon = 0.0 ? modim_min gt epsilon : epsilon
          ivar_weight0 = alog10((maskstruc.bsplinecen.modim+epsilon)/(modim_min+epsilon))
          ...
          if (niter eq 1) then spawn, "touch "+acsproc_datafile(metastruc_visit[q],/photmod,ext='e')
          dspars = devpars
