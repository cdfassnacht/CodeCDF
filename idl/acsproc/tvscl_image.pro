...
;
; PURPOSE:
;	This routine is an implementation of the exsited routine 'TVIMAGE' so 
;	that it can scale the input image properly. 
; AUTHOR:
...
;               set or !P.MULTI is set to multiple plots and TVSCL_IMAGE is
...
;               used.
;
;     TV:       Setting this keyword makes the TVSCL_IMAGE command work much
...
;     function. The TVSCL_IMAGE command will load the color table vectors
...
;        FREE_LUN, lun
;
;        TVSCL_IMAGE, image, POSITION=thisPosition, /KEEP_ASPECT_RATIO
...
;         using TVSCL_IMAGE. Added 24-bit image capability. 15 April 1997. DWF.
...
;      Added TV keyword to allow TVSCL_IMAGE to work like the TV command. 11 May 99. DWF.
...
if((not keyword_set(min_image)) or (not keyword_set(max_image))) then begin   
   ; Rescale the image so that the minimum value is 0 and the maximum is 255
   if (keyword_set(mask)) then begin
      med = median(image*mask)
      sig = stddev(image*mask)
      max_image = (med + (10 * sig)) < max(image*mask)
      min_image = (med - (2 * sig))  > min(image*mask)
      if (finite(min_image) EQ 0) then min_image = min(image*mask)
      if (finite(max_image) EQ 0) then max_image = max(image*mask)

      if (min_image GE max_image) then begin
         min_image = min_image - 1
         max_image = max_image + 1
      endif
   endif else begin
      med = median(image)
      sig = stddev(image)
      max_image = (med + (10 * sig)) < max(image)
      min_image = (med - (2 * sig))  > min(image)
      if (finite(min_image) EQ 0) then min_image = min(image)
      if (finite(max_image) EQ 0) then max_image = max(image)
...
image1=bytscl(image, /nan, min=min_image, max=max_image, top=247)
...
IF s[0] EQ 3 THEN BEGIN
   IF (s[1] NE 3L) AND (s[2] NE 3L) AND (s[3] NE 3L) THEN BEGIN
      ...
      ENDIF ELSE BEGIN
         ...
         CASE single OF
            1: image1 = Reform(image1, s[2], s[3])
            2: image1 = Reform(image1, s[1], s[3])
            3: image1 = Reform(image1, s[1], s[2])
         ENDCASE
         s = Size(image1)
      ENDELSE
   ENDIF
ENDIF ELSE s = Size(image1)
...
IF Keyword_Set(tv) THEN BEGIN
   ...
   IF N_Params() GE 3 OR N_Params() EQ 1 THEN BEGIN
     ...
     IF Keyword_Set(normal) THEN TV, image1, x, y, True=true, _Extra=extra, /Normal ELSE $
                                 TV, image1, x, y, True=true, _Extra=extra, /Device
   ENDIF ELSE BEGIN
     IF N_Params() EQ 2 THEN BEGIN
         IF Keyword_Set(normal) THEN TV, image1, x, True=true, _Extra=extra, /Normal ELSE $
                                     TV, image1, x, True=true, _Extra=extra, /Device
     ENDIF
   ENDELSE
...
IF KEYWORD_SET(keep) THEN BEGIN
      ...
      ; Find aspect ratio of image1.

   ratio = FLOAT(imgYsize) / imgXSize

      ; Find the proposed size of the image1 in pixels without aspect
      ; considerations.
...
IF (!D.Flags AND 1) NE 0 THEN BEGIN
   ...
   TV, image1, xstart, ystart, XSIZE=xsize, $
      YSIZE=ysize, _EXTRA=extra, True=true

ENDIF ELSE BEGIN ; All other devices.

   CASE true OF
      0: TV, TVIMAGE_CONGRID(image1, xsize, ysize, INTERP=interp, $
            MINUS_ONE=minusOne, HALF_HALF=half_half), xstart, $
            ystart, _EXTRA=extra
      1: IF thisDepth GT 8 THEN BEGIN
            TV, TVIMAGE_CONGRID(image1, 3, xsize, ysize, INTERP=interp, $
               MINUS_ONE=minusOne, HALF_HALF=half_half), xstart, $
               ystart, _EXTRA=extra, True=1
         ENDIF ELSE BEGIN
            image2d = Color_Quan(image1, 1, r, g, b, _Extra=extra)
            TVLCT, r, g, b
      ...
      2: IF thisDepth GT 8 THEN BEGIN
            TV, TVIMAGE_CONGRID(image1, xsize, 3, ysize, INTERP=interp, $
               MINUS_ONE=minusOne, HALF_HALF=half_half), xstart, $
               ystart, _EXTRA=extra, True=2
         ENDIF ELSE BEGIN
            image2d = Color_Quan(image1, 2, r, g, b, _Extra=extra)
            TVLCT, r, g, b
            TV, TVIMAGE_CONGRID(image2d, xsize, ysize, INTERP=0, $
               MINUS_ONE=minusOne, HALF_HALF=half_half), xstart, $
               ystart, _EXTRA=extra, True=0
      ...
      3: IF thisDepth GT 8 THEN BEGIN
            TV, TVIMAGE_CONGRID(image1, xsize, ysize, 3, INTERP=interp, $
               MINUS_ONE=minusOne, HALF_HALF=half_half), xstart, $
               ystart, _EXTRA=extra, True=3
         ENDIF ELSE BEGIN
            image2d = Color_Quan(image1, 3, r, g, b, _Extra=extra)
            TVLCT, r, g, b
            TV, TVIMAGE_CONGRID(image2d, xsize, ysize, INTERP=0, $
               MINUS_ONE=minusOne, HALF_HALF=half_half), xstart, $
               ystart, _EXTRA=extra, True=0
