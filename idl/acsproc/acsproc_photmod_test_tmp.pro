...
pro acsproc_photmod_test_tmp, visit, flatweight=flatweight, cycle=cycle,program=program, doplot=doplot, fname=fname

;  common acsproc_photmod_data
  append = 0
  !Except = 0
;.com acsproc_photmod_test_tmp
common acsproc_photmod_data, acs_filter, dataroot
;cycle=14
;program=10174
;visit='16'
...
;        splog, "READING: "+parfile
        tmp=mrdfits(fname, 1, /silent)
        splog, "reading: "+fname
        cspars=[dspars, (tmp.pars)[-4:-1]]
        if (cspars[-4] lt 0.1) then cspars[-4]=1.0
...
;        parinfo[-1].fixed=1
        ...
        cspars=[dspars, 4.0, 0.5, 20.0, 1.0]
        parinfo=replicate({value: 0.D, fixed: 0, limited:[0, 0], limits: [0.0, 0.0]}, n_elements(cspars)) &$
        parinfo.value=cspars
;        parinfo[2].fixed=1
;        parinfo[3].fixed=1
...
;        parinfo[-1].fixed=1
      ...
      if keyword_set(plot) then $
        cspars = mpfit2dfun(fitfunc[2], dimg, aimg, $
      ...
      ;outermask = ((img-yfit)*sqrt(ivar)) ge 4.0
