pro cand_2x2, candlist, prefix=prefix, rsddir=rsddir, fitdir=fitdir, $
...
;               Like cand_stack, but instead of each line containing
;               4 images associated with one target (BVi,z,fit,resid),
;               each line has 4 images, with 2 each for 2 targets
;               (BVi and resid).  So, for N input galaxies, the
;               output is a 4 x N/2 grid of panels.
;
...
if n_params() lt 1 then begin
    ...
    print, ''
    print, 'syntax: cand_2x2, candlist [,prefix=prefix,,rsddir=rsddir,'
...
ycells = replicate(1,neid/2)
subcellarray, [1,1,1,1], ycells, newpan, newsubpan
ysize = 1.0 * (neid/2) * psfigsz + psbordsz
...
for i=0,((neid/2)-1) do begin
   ...
   print, ''
   print, 'cand_2x2: Reading input files:'
   print, 'cand_2x2:   ',bfits1
   bim1 = mrdfits(bfits1, 0, bhead1)
   print, 'cand_2x2:   ',vfits1
   vim1 = mrdfits(vfits1, 0, vhead1)
   print, 'cand_2x2:   ',ifits1
   iim1 = mrdfits(ifits1, 0, ihead1)
   print, 'cand_2x2:   ',rsd1fits
   rsd1im = mrdfits(rsd1fits, 0, rsd1head)
   print, 'cand_2x2:   ',bfits2
   bim2 = mrdfits(bfits2, 0, bhead2)
   print, 'cand_2x2:   ',vfits2
   vim2 = mrdfits(vfits2, 0, vhead2)
   print, 'cand_2x2:   ',ifits2
   ...
   setup.lab3 = 'BVi'
   setup.lab4 = 'Resid'
   if (i eq (neid/2)-1) then setup.doxlab = doxlab else setup.doxlab = 0
   ...
   if (i eq 0) then begin
      plot_2x2, bvi1, reimsc1,bvi2, reimsc2, $
