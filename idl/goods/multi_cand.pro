pro multi_cand, catfile, istart, prefix=prefix, _extra=_extra

;
; Procedure multi_cand
;
; Description: Interactive interface for calling cand_plot.pro.  The
...
;
; Revision history:
;  2003Mar27 Chris Fassnacht -- A modification multi_cand.pro
...
if n_params() lt 1 then begin
    print, ''
    print, $
      'syntax: multi_cand, catfile, (istart) [,prefix=prefix,_extra=_extra]'
    print, ''
    return
endif
...
if istart lt 0 or istart gt (neid-1) then begin
   print, ''
   print,'ERROR: multi_cand.  Need to give start value in range 0 --'+$
      strcompress(neid-1)
   print, '*** Exiting ***'
   print, ''
   return
