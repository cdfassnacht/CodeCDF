pro multi_gdsbviz, catfile, istart, prefix=prefix, _extra=_extra

;
; Procedure multi_gdsbviz
;
; Description: Interactive interface for calling goods_bviz.pro.  The
...
;
; Revision history:
;  2003Mar27 Chris Fassnacht -- A modification multi_gdsbviz.pro
...
if n_params() lt 1 then begin
    print, ''
    print, $
      'syntax: multi_gdsbviz, catfile, (istart) [,prefix=prefix,_extra=_extra]'
    print, ''
    return
endif
...
if istart lt 0 or istart gt (neid-1) then begin
   print, ''
   print,'ERROR: multi_gdsbviz.  Need to give start value in range 0 --'+$
      strcompress(neid-1)
   print, '*** Exiting ***'
   print, ''
   return
...
while mousebutt ne 2 do begin
   if(i ge 0 and i lt neid) then begin
      ...
      case strlen(basestr) of
       ...
       goods_bviz, idstr, _extra=_extra
   endif else begin
