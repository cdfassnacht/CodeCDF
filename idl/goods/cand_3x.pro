pro cand_3x, candlist, prefix=prefix, rsddir=rsddir, fitdir=fitdir, $
...
;               each row has 3 images, one for each target (BVi only).
;               So, for N input galaxies, the
;               output is a  3 x N/3 grid of panels.
;
...
;
; Revision history:
;  2003Jun29 Chris Fassnacht -- A slight modification of cand_5x.pro
;
...
;   print, '   page in postscript mode.'
    print, ''
    print, 'syntax: cand_3x, candlist [,prefix=prefix,,rsddir=rsddir,'
...
readcol, candlist, id, format='A', comment='#'
neid=n_elements(id)

; Set up viewport window 

ycells = replicate(1,neid/3)
subcellarray, [1,1,1], ycells, newpan, newsubpan
ysize = 1.0 * (neid/3) * psfigsz + psbordsz
...
for i=0,(neid-1) do begin
   ...
   print, ''
   print, 'cand_3x: Reading input files:'
   print, 'cand_3x:   ',bfits1
   bim1 = mrdfits(bfits1, 0, bhead1)
   print, 'cand_3x:   ',vfits1
   vim1 = mrdfits(vfits1, 0, vhead1)
   print, 'cand_3x:   ',ifits1
