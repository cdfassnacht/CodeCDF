...
int main(int argc, char *argv[])
{
  ...
  float *fluxp=NULL;      /* Component fluxes from phasecal model */
  float *fluxg=NULL;      /* Component fluxes from gscale model */
  ...
  float *pptr,*gptr;      /* Pointers to navigate flux arrays */
  ...
  if(argc != 3) {
    ...
    fprintf(stderr,"and the source root name is the first 4 digits of the\n");
    fprintf(stderr,"source name.  For example:\n\n");
    fprintf(stderr,"             getflux 20000214 1608\n\n");
    return 1;
  }

  /*
   * Set datename and root variables from command line
   */
  ...
  /*
   * Set other variables based on value of root
   */
  ...
  else if(strcmp(root,"0414") == 0) {
    srctype = LENS;
    ncomp = 4;
  }
  else if(strcmp(root,"0712") == 0) {
    srctype = LENS;
    ncomp = 4;
  }
  else if(strcmp(root,"1030") == 0) {
    srctype = LENS;
    ncomp = 2;
  }
  else if(strcmp(root,"1127") == 0) {
    srctype = LENS;
    ncomp = 2;
  }
  else if(strcmp(root,"1152") == 0) {
    srctype = LENS;
    ncomp = 2;
  }
  else {
  ...
  /*
   * Allocate memory for source flux densities
   */

  if(no_error)
    if(!(fluxp = new_array(ncomp,1)))
      no_error = 0;

  if(no_error)
    if(!(fluxg = new_array(ncomp,1)))
      no_error = 0;
  ...
  printf("\n");
  if(no_error)
    if((source_flux(datename,root,"p",fluxp,&rmsp,&chisqp,
		    &obsdate,ncomp)) == 1)
      no_error = 0;

  if(no_error)
    if((source_flux(datename,root,"g",fluxg,&rmsg,&chisqg,
		    &obsdate,ncomp)) == 1)
      no_error = 0;
  ...
  if(no_error) {
    ...
    for(i=0,pptr=fluxp,gptr=fluxg; i<ncomp; i++,pptr++,gptr++) {
      ...
      printf("   %4d         %7.3f     %7.3f     %6.3f  %7.3f\n",
	     i+1,*pptr,*gptr,diff,100.0*diff / *gptr);
    }
  ...
  if(srctype == LENS) {
    if(!(dayfp = open_writefile("tmpday")))
      no_error = 0;
    sprintf(baddayname,"%sbadday.edt",REDDIR);
